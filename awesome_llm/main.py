...
models = {
    # GPT_v1: Basic model with a simple embedding layer, demonstrating the foundational structure.
    "GPT_v1": GPT_v1(vocab_size=vocab_size),
    
    # GPT_v2: Adds a linear transformation layer on top of the embeddings, introducing the ability to learn more complex mappings.
    "GPT_v2": GPT_v2(vocab_size=vocab_size, n_embd=32),

    # GPT_v3: Incorporates positional embeddings, allowing the model to understand sequence order, crucial for text processing.
    "GPT_v3": GPT_v3(vocab_size=vocab_size, n_embd=32, block_size=8),

    # GPT_v4: Introduces a single head of self-attention, marking the beginning of the model's capacity for contextual understanding.
    "GPT_v4": GPT_v4(vocab_size=vocab_size, n_embd=32, block_size=8, head_size=32, dropout=0.1),

    # GPT_v5: Expands to multi-head attention, enabling the model to attend to different parts of the sequence simultaneously.
    "GPT_v5": GPT_v5(vocab_size=vocab_size, n_embd=32, block_size=8, head_size=8, dropout=0.1, num_heads=4),

    # GPT_v6: Add a FeedForward (computation block)
    "GPT_v6": GPT_v6(vocab_size=vocab_size, n_embd=32, block_size=8, head_size=8, dropout=0.1, num_heads=4),

    # GPT_v7: Adds multiple layers of Transformer blocks, each block includes MultiHeadAttention and FeedForward with residual connections and layer normalization.
    "GPT_v7": GPT_v7(vocab_size=vocab_size, n_embd=32, block_size=8, dropout=0.1, num_heads=4, num_layers=3),

    # GPT_final: GPT_v7 scaled up
    "GPT_final": GPT_v7(vocab_size=vocab_size, n_embd=384, block_size=256, dropout=0.2, num_heads=6, num_layers=6)
}
