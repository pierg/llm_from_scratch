...
class GPT_v4(nn.Module):
    ...
    def __init__(self, vocab_size: int, n_embd: int, block_size: int, head_size: int, dropout: float = 0.1) -> None:
        """
        ...
        # Since we only have one head, we can set the head_size to be equal to n_embd
        assert head_size == n_embd, "Head size must be equal to the embedding size for a single head self-attention"
        self.sa_head = Head(head_size=head_size, n_embd=n_embd, block_size=block_size, dropout=dropout)
    ...
    def forward(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        position_indices = torch.arange(T, device=device).expand(B, -1)  # Position indices for the sequence
